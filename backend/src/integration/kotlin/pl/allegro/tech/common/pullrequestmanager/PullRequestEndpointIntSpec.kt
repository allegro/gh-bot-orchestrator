...
import pl.allegro.tech.common.pullrequestmanager.PullRequestEndpointIntSpec.Companion.BODY
import pl.allegro.tech.common.pullrequestmanager.PullRequestEndpointIntSpec.Companion.BRANCH
import pl.allegro.tech.common.pullrequestmanager.PullRequestEndpointIntSpec.Companion.COMMIT
import pl.allegro.tech.common.pullrequestmanager.PullRequestEndpointIntSpec.Companion.PR_NUMBER
import pl.allegro.tech.common.pullrequestmanager.PullRequestEndpointIntSpec.Companion.PR_TITLE
import pl.allegro.tech.common.pullrequestmanager.PullRequestEndpointIntSpec.Companion.SC_ID
...
class PullRequestEndpointIntSpec(val rest: MockMvc) : BaseIntegrationSpec() {
    ...
    init {
            ...
            val dependabotWorkflow = workflowDefinitionFixtures.randomWorkflow(
                filters = CompoundPullRequestMatcher(
                    ...
                    matchers = listOf(
                        ...
                        DependabotMatcher(
                            pattern = "pl.allegro.tech.common:andamio-starter.*",
                        )
                    )
                )
            )
            ...
            eventually(5.seconds) {
                githubApiMock.verifyWorkflowDispatch(
                    dependabotWorkflow, ExtraParams(
                        listOf(
                            VersionUpdate(
                                "pl.allegro.tech.common:andamio-starter-dependencies", Version.parse("6.2.0"),
                                Version.parse("8.1.0")
                            )
                        ), null
                    )
        ...
        test("should not trigger workflow for dependabot PRs when dependabot filter does not match") {
            ...
            rest.post("/pull-requests") {
                contentType = MediaType.APPLICATION_JSON
                content = examplePullRequestEvent(body = "Bumps opbox-build-tools from 6.2.0 to 8.1.0.")
            }.andExpect {
                status { isOk() }
            }
    ...
    companion object {
        ...
        const val PR_TITLE = "Bump pl.allegro.tech.common:andamio-starter-dependencies from 7.2.0 to 8.0.0"
        const val SC_ID = "sc-2137"
        const val COMMIT = "d1a6c13bff04dc0f090f97c57a61ddd9f8af1816"
        const val BRANCH = "some-branch-with-pr"
        const val BODY = "Bumps pl.allegro.tech.common:andamio-starter-dependencies from 6.2.0 to 8.1.0."
    }
}
